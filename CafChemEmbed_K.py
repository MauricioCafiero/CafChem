...
class embed_training_data():
  ...
  def read_with_classes(self, file_path: str, SMILES_column: str, classes_column: str,
                        classes_to_contrast: list[int], test_size = 0.15, output_path=''):
    '''
      ...
      return a dataframe and a split dataset. Cleans ions from the SMILES strings.
        ...
        Args:
          ...
          output_path: path where output files will be saved (optional)
        Returns:
    ...
    f = open(f'{output_path}data_pairs.csv', 'w')
    ...
    f.close()
    pairs_df = pd.read_csv(f'{output_path}data_pairs.csv')

    pairs_ds = load_dataset('csv', data_files=f'{output_path}data_pairs.csv')
  ...
  def read_make_classes(self, file_path: str, target_name: str, 
    ...
    df.to_csv(f"{output_path}data_classes.csv", index = False)

    pairs_df, pairs_ds = self.read_with_classes(f"{output_path}data_classes.csv", smiles_name, 
